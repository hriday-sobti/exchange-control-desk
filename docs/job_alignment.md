# Target Job Alignment Matrix: Exchange Control Desk

This document compares the operational responsibilities and analytical requirements expected of a Data Analyst / Risk & Operations Analyst at a premier Virtual Digital Asset (VDA) exchange (modeled on public CoinDCX requirements) with the concrete capabilities built into **Exchange Control Desk**.

---

## Analytical & Business Alignment Matrix

| Target Job Requirement / Skill | Industry / Business Context | How Exchange Control Desk Demonstrates It | Concrete Code & Artifact Evidence |
| :--- | :--- | :--- | :--- |
| **Advanced SQL & Query Optimization** | Extracting operational KPIs, cohorting active users, calculating rolling metrics and funnel drop-offs across millions of records. | Implements a robust SQL transformation layer using CTEs, window functions (`ROW_NUMBER`, `DENSE_RANK`, `LAG`, `LEAD`, rolling 24h windows), and conditional aggregations. | `sql/transformations/user_activity.sql`<br>`sql/transformations/rolling_metrics.sql`<br>`sql/views/analytical_views.sql`<br>`sql/kpis/executive_kpis.sql` |
| **Python for Data Pipelines & Statistical Analysis** | Ingesting external APIs, validating ingestion pipelines, computing statistical baselines, and automating risk detection. | End-to-end modular Python architecture: API fetchers, data cleaning, Z-score / IQR calculations, rolling user baselines, and evaluation engines. | `python/ingestion/`<br>`python/cleaning/`<br>`python/feature_engineering/`<br>`python/anomaly_detection/` |
...
| **Business Intelligence & Executive Storytelling** | Communicating operational status, platform liquidity, risk exposures, and data integrity to senior executives. | 5-Page Power BI architecture with comprehensive DAX measures, star-schema semantic model, and executive summary reports. | `powerbi/measures/measures.dax`<br>`docs/dashboard_guide.md`<br>`outputs/executive/executive_summary.md` |
...
| **Reproducibility & Engineering Rigor** | Ensuring code is maintainable, tested, documented, and easily deployed across environments. | Full suite of unit, integration, and failure tests; single CLI runner (`python scripts/run_pipeline.py`); configuration via YAML and `.env`. | `tests/`<br>`scripts/run_pipeline.py`<br>`config/project_config.yaml` |

---

## Analytical Maturity Demonstrated
...
3. **Predictive / Detection:** Which behaviors diverge statistically from historic baselines? (Rolling 30-day Z-scores, IQR outlier detection).
